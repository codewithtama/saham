...
# Timeout (detik) untuk setiap operasi pengambilan data dari yfinance
_TIMEOUT_DATA = 15        # ambil_data per ticker
_TIMEOUT_FUNDAMENTAL = 10  # ambil_fundamental per ticker
_TIMEOUT_MARQUEE = 20     # ambil_data_marquee (bulk download semua ticker)


# ========================================
# FUNGSI AMBIL DATA
# ========================================
def _fetch_data_worker(ticker: str, periode: str) -> pd.DataFrame | None:
    """Fungsi internal pengambil data — dijalankan di thread terpisah."""
    if periode == "1d":
        intv = "5m"
    elif periode == "5d":
        intv = "15m"
...
def ambil_data(ticker: str, periode: str) -> pd.DataFrame | None:
    try:
        with ThreadPoolExecutor(max_workers=1) as executor:
            future = executor.submit(_fetch_data_worker, ticker, periode)
            return future.result(timeout=_TIMEOUT_DATA)
    except FuturesTimeoutError:
        st.warning(f"Timeout saat mengambil data {ticker}. Coba lagi nanti.")
        return None
    except Exception as e:
...
def _fetch_fundamental_worker(ticker: str) -> dict:
    """Fungsi internal pengambil data fundamental — dijalankan di thread terpisah."""
    t = yf.Ticker(ticker)
    info = t.info
    if not info:
        return {}
...
def ambil_fundamental(ticker: str) -> dict:
    try:
        with ThreadPoolExecutor(max_workers=1) as executor:
            future = executor.submit(_fetch_fundamental_worker, ticker)
            return future.result(timeout=_TIMEOUT_FUNDAMENTAL)
    except FuturesTimeoutError:
        return {}
    except Exception:
        return {}
...
def _fetch_marquee_worker() -> list:
    """Fungsi internal pengambil data marquee — dijalankan di thread terpisah."""
    tickers = list(SAHAM_IDX.values())
    tickers_str = " ".join(tickers)

    df_all = yf.download(
        tickers_str, period="2d", auto_adjust=True, progress=False,
        timeout=_TIMEOUT_MARQUEE,
    )
    if df_all.empty:
        return []
...
@st.cache_data(ttl=300)
def ambil_data_marquee() -> list | None:
    """
    Mengambil data harga terkini untuk semua saham di SAHAM_IDX.
    Mengembalikan None jika timeout atau gagal, sehingga marquee bisa dilewati
    tanpa menghentikan render halaman.
    """
    try:
        with ThreadPoolExecutor(max_workers=1) as executor:
            future = executor.submit(_fetch_marquee_worker)
            return future.result(timeout=_TIMEOUT_MARQUEE)
    except FuturesTimeoutError:
        return None
    except Exception:
        return None
