...
def _hitung_sharpe(
    equity_history: list[float], risk_free_rate: float = 0.06
) -> float:
    """
    Sharpe Ratio annualized dari equity history harian.
    Bunga bebas risiko default 6% = estimasi rata-rata SBI/SBN Indonesia.
    Interpretasi: > 1 bagus, > 2 sangat bagus, < 0 strategi lebih buruk dari deposito.
    """
...
def jalankan_backtest(df: pd.DataFrame, initial_capital: float = 10000000.0) -> dict:
    ...
    # Temukan bar pertama di mana SEMUA indikator valid (tidak NaN)
    start_idx = 0
    ...
    for i in range(start_idx, len(df)):
        ...
        # 1. RSI — level-based: oversold/overbought
        ...
        # 2. MA Position — trend filter: posisi MA20 vs MA50
        ...
        # 3. MACD Crossover — entry timing: hanya saat MACD memotong Signal Line
        #    (lebih tepat sebagai trigger masuk/keluar vs. hanya cek posisi relatif)
        if i > start_idx:
            prev_m = float(macd.iloc[i - 1])
            ...
            if prev_m <= prev_s and curr_m > curr_s:
                buy_indicators += 1   # Bullish MACD crossover
            elif prev_m >= prev_s and curr_m < curr_s:
                sell_indicators += 1  # Bearish MACD crossover

        # 4. Stochastic — level + direction: reversal di zona ekstrem
        st_k = float(k_line.iloc[i])
        st_d = float(d_line.iloc[i])
        if st_k < 20 and st_k > st_d:
            buy_indicators += 1
        elif st_k > 80 and st_k < st_d:
            sell_indicators += 1

        # Beli/jual hanya kalau minimal 2 tanda searah dan zero konflik
        daily_signal = "HOLD"
        ...
        # Stop-loss: jual paksa jika harga turun >= STOP_LOSS_PCT dari harga beli
        # Ini override sinyal indikator — proteksi modal lebih penting
        if in_position and close_price <= buy_price * (1 - STOP_LOSS_PCT):
            daily_signal = "SELL"
        ...
        if not in_position and daily_signal == "BUY":
            ...
            net_proceeds = shares * close_price * (1 - BROKER_FEE_SELL)
    ...
    # Catat posisi yang masih terbuka di akhir periode ke dalam trade log
    # agar user bisa lihat unrealized P&L di tabel transaksi
    if in_position:
        unrealized_proceeds = shares * final_close * (1 - BROKER_FEE_SELL)
        unrealized_pnl_rp = unrealized_proceeds - buy_cost_basis
        unrealized_pnl_pct = (unrealized_pnl_rp / buy_cost_basis) * 100
        trades.append(
            {
                "tanggal_beli": buy_date,
                "tanggal_jual": df.index[-1],
                "harga_beli": buy_price,
                "harga_jual": final_close,
                "return_pct": unrealized_pnl_pct,
                "profit_rp": unrealized_pnl_rp,
                "lembar": shares,
                "masih_buka": True,
            }
        )
    ...
    # Win rate hanya dari transaksi yang sudah ditutup (bukan posisi masih buka)
    closed_trades = [t for t in trades if not t.get("masih_buka")]
    profit_trades = [t for t in closed_trades if t["return_pct"] > 0]
    win_rate = (len(profit_trades) / len(closed_trades)) * 100 if closed_trades else 0.0
    ...
    sharpe = _hitung_sharpe(equity_history)
